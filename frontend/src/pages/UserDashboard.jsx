...
import { Mail, Shapes, Phone, Loader, CreditCard, Calendar, BookText } from "lucide-react";
...
const UserDetails = ({ name, email, picture, phoneNo, subscriptionId, expiryDate,currentChapterForWhatsapp,currentCourseForWhatsapp }) => (
...
const CustomLabel = ({ viewBox, value1, value2, value3 }) => {
  ...
  return (
    <text x={cx} y={cy} fill="#333" textAnchor="middle" dominantBaseline="central">
      <tspan x={cx} dy="-1.2em" fontSize="24" fontWeight="bold">{value1 + value2 + value3}</tspan>
      <tspan x={cx} dy="1.6em" fontSize="14">Total</tspan>
    </text>
  );
};
...
const UserStats = ({ questionStats }) => {
  ...
  return (
    <div className="bg-indigo-100 w-full lg:w-1/2 lg:ml-4 p-6 rounded-lg shadow-md">
      <h3 className="text-2xl font-bold mb-4 text-indigo-900">Question Statistics</h3>
      <div className="flex flex-col md:flex-row items-center justify-between">
        ...
        <div className="w-full md:w-2/3 md:pl-6">
          ...
          <div className="mt-6 text-center">
            ...
            <p className="text-3xl font-bold text-indigo-600">{totalAnswered} / {TOTAL_QUESTIONS * 3}</p>
          </div>
        </div>
      </div>
    </div>
